...
bool	check_o_v_k_l_modes(int &pos, std::string modes, char c)
{
	if (c == '+')
	{
		if (pos == 0) // Mode +o
...
void	Server::add_remove_mode_to_channel(Channel &channel, msg_parse &command, User &user, char c)
{
	...
	for (size_t i = 0; i < mode.size(); i++)
	{
		...
		else
		{
			...
			{
				if (check_o_v_k_l_modes(pos, command.get_cmd_params()[1], c)) // Checking if there's any combination of o,k,v,l except ov. 
				{
